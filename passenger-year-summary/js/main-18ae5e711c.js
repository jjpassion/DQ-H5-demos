!function(){
	...
	var app = window.app = app||{
		...
		dataAll:0,
		event:[
			{id:'shareTimes',desc:'分享'},
			{id:'clickPlay',desc:'点击开启盘点'},
			{id:'panPage1',desc:'滑动婴儿车到下一页'},
			{id:'panPage2',desc:'滑动白天老爷车到下一页'},
			{id:'panPage3',desc:'滑动夜里老爷车到下一页'},
			{id:'panPage4',desc:'滑动雨天鲸鱼到下一页'},
			{id:'panPage5',desc:'滑动飞行器到下一页'},
			{id:'panPage6',desc:'滑动骑士到下一页'},
			{id:'panPage7',desc:'滑动最后婴儿车到落版页'},
			{id:'clickPage7',desc:'点击最后婴儿车到落版页'}
			]
	};
	...
	var initAction = function(){
		...
		Frame.init('canvas',540);
		Frame.drawBitmap();

		f.setBASE('image/data_0-e014dfc6c4.json',function(base){

			app.cartoonGo = new Frame.bitmap({base:base,sum:f.isLow?0:127,ftp:0.5});
		...
		f.setBASE('image/data_1-aeec54ae20.json',function(base){
			...
			app.cartoonLoop = new Frame.bitmap({base:base,sum:f.isLow?0:21});
		...
		f.setBASE('image/data_2-029ed6539c.json',function(base){
			...
			app.AGo = new Frame.bitmap({base:base,sum:f.isLow?0:26,ftp:.6});
		...
		f.setBASE('image/data_3-db8b408e8b.json',function(base){
			...
			app.ALoop = new Frame.bitmap({base:base,sum:f.isLow?0:72});
		...
		f.setBASE('image/data_4-99712d0f2e.json',function(base){
			...
			app.BLoop = new Frame.bitmap({base:base,sum:f.isLow?0:72});
		...
		f.setBASE('image/data_5-187d541dfe.json',function(base){
			...
			app.CLoop = new Frame.bitmap({base:base,sum:f.isLow?0:72});
		...
		f.setBASE('image/data_6-38108fa9d5.json',function(base){
			...
			app.DLoop = new Frame.bitmap({base:base,sum:f.isLow?0:72});
		...
		f.setBASE('image/data_7-4ef2c50281.json',function(base){
			...
			app.ELoop = new Frame.bitmap({base:base,sum:f.isLow?0:72});
		...
		f.setBASE('image/data_8-376383c043.json',function(base){
			...
			app.FLoop = new Frame.bitmap({base:base,sum:f.isLow?0:72});
